...
export function makeAccountApi(ai: ApiInput, accountId: string): EdgeAccount {
  ...
  const out: EdgeAccount = {
    ...
    async changePin(opts: {
      ...
      return await changePin(ai, accountId, pin, enableLogin).then(() => {
        const { login } = selfState()
    ...
    async changeRecovery(
      ...
      lockdown()
      return await changeRecovery(ai, accountId, questions, answers).then(
        () => {
          const { loginTree } = selfState()
    ...
    async approveVoucher(voucherId: string): Promise<void> {
      return await changeVoucherStatus(ai, loginTree, login, {
